...
def import_to_vnpy(df: pd.DataFrame, symbol: str, exchange: Exchange):
    ...
    for idx, row in df.iterrows():
        ...
        bar = BarData(
            ...
            open_price=float(row.get('open', row.get('开盘价', 0))),
            high_price=float(row.get('high', row.get('最高价', 0))),
            low_price=float(row.get('low', row.get('最低价', 0))),
            close_price=float(row.get('close', row.get('收盘价', 0))),
            open_interest=0,
